...
def _generate_default_edges(files: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def _fallback_metadata_plan(project_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if features:
        ...
        base_name = _slugify(project_spec.get("title", "app"))
    ...
    edges = _generate_default_edges(files)

    return {"files": files, "edges": edges}


def _sanitize_plan(plan_data: Dict[str, Any], project_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    default_extension = _infer_default_extension(project_spec)
    ...
    for entry in raw_files:
        ...
        node_id = _slugify(str(base_id_source))
    ...
    if not sanitized_edges:
        sanitized_edges = _generate_default_edges(sanitized_files)
...
def _plan_workspace_with_letta(project_spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for message in response.messages:
        if message.message_type == "assistant_message" and message.content:
            try:
                plan_data = _extract_structured_payload(message.content)
                return _sanitize_plan(plan_data, project_spec)
